...
import {
  buildFileName, buildNonProgrammaticName, titleFrom, LONG_NAME_WARNING_LENGTH,
} from './formatter.js';
...
/** Filesystem timestamp (ms) -> the literal calendar date, local time. */
...
/**
 * A file is programmatic when we know both the program and the partner — that
 * is what the guidance's five-element name requires. Without them the file is
 * treated as non-programmatic and named "Title - YYYY-MM-DD - Stage" instead,
 * rather than being skipped.
 */
function isProgrammatic(meta) {
  return Boolean(meta.programAcronym && meta.leadPartnerName);
}

function missingFields(meta) {
  const missing = [];
  if (isProgrammatic(meta) && !meta.documentType) missing.push('DocumentType');
...
export async function buildPlan(directoryHandle, options, onProgress) {
  ...
  for (const entry of entries) {
    ...
    // Date sources, best first. The internal date says when the document was
    // written; the name usually repeats a date an earlier run already derived;
    // the filesystem date only says when the file was last touched on disk —
    // a copy, a sync or an antivirus scan all move it. Anything below the
    // first source is flagged in the preview so a weak date is never silently
    ...
    if (!documentDate && entry.file) {
      documentDate = dateFromTimestamp(entry.file.lastModified);
      if (documentDate) {
        dateNote = 'no internal date; using the Windows modified date, which reflects the last'
                 + ' time the file was touched on disk rather than when it was written';
      }
    }
    ...
    const stem = entry.name.slice(0, entry.name.lastIndexOf('.') > 0
      ? entry.name.lastIndexOf('.') : undefined);

    const programmatic = isProgrammatic(meta);
    const proposedName = programmatic
      ? buildFileName(meta, extensionOf(entry.name))
      : buildNonProgrammaticName(titleFrom(stem), meta, extensionOf(entry.name));
    ...
    plans.push({
      ...
      notes: [
        programmatic ? '' : 'non-programmatic name (no Program-Partner folder)',
        dateNote,
        longName,
      ].filter(Boolean).join('  |  '),
    });
